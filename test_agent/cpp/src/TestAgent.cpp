...
TestAgent::TestAgent(const std::string transportType) {
	spdlog::info("TestAgent::TestAgent(), Creating TestAgent with transport type: {}", transportType);
	...
	actionHandlers_ = {
	    {string(Constants::SEND_COMMAND),
	     std::function<UStatus(Document &)>([this](Document &doc) { return this->handleSendCommand(doc); })},
	    {string(Constants::REGISTER_LISTENER_COMMAND), std::function<UStatus(Document &)>([this](Document &doc) {
		     return this->handleRegisterListenerCommand(doc);
	     })},
	    {string(Constants::UNREGISTER_LISTENER_COMMAND), std::function<UStatus(Document &)>([this](Document &doc) {
		     return this->handleUnregisterListenerCommand(doc);
	     })},
	    {string(Constants::INVOKE_METHOD_COMMAND),
	     std::function<void(Document &)>([this](Document &doc) { this->handleInvokeMethodCommand(doc); })},
	    {string(Constants::SERIALIZE_URI),
	     std::function<void(Document &)>([this](Document &doc) { this->handleSerializeUriCommand(doc); })},
	    {string(Constants::DESERIALIZE_URI),
	     std::function<void(Document &)>([this](Document &doc) { this->handleDeserializeUriCommand(doc); })}};
}
...
UStatus TestAgent::onReceive(uprotocol::utransport::UMessage &transportUMessage) const {
	...
	if (transportUMessage.attributes().type() == uprotocol::v1::UMessageType::UMESSAGE_TYPE_REQUEST) {
		...
		uprotocol::utransport::UPayload payload1((const unsigned char *) serialized_message.c_str(),
		...
		auto attr = uprotocol::utransport::UAttributesBuilder::response(
		                transportUMessage.attributes().sink(), transportUMessage.attributes().source(),
...
Value TestAgent::createRapidJsonStringValue(Document &doc, const std::string &data) const {
	Value stringValue(rapidjson::kStringType);
	stringValue.SetString(data.c_str(), doc.GetAllocator());
	return stringValue;
}

void TestAgent::writeDataToTMSocket(Document &responseDoc, const std::string action) const {
...
void TestAgent::sendToTestManager(const Message &proto, const string &action, const string &strTest_id) const {
	...
	if (!strTest_id.empty()) {
		...
		jsonStrValue.SetString(strTest_id.c_str(), static_cast<rapidjson::SizeType>(strTest_id.length()),
...
void TestAgent::sendToTestManager(Document &document, Value &jsonData, const string action,
...
UStatus TestAgent::handleSendCommand(Document &jsonData) {
	...
	auto uAttributesWithId = uprotocol::utransport::UAttributesBuilder(
	                             umsg.attributes().source(), id, umsg.attributes().type(), umsg.attributes().priority())
	                             .build();
