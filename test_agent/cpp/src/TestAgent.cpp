...
TestAgent::TestAgent(std::string transportType) {
	...
	clientSocket_   = 0;
	actionHandlers_ = {{string(Constants::SEND_COMMAND), std::function<UStatus(Document &)>([this](Document &doc) {
		                    return this->handleSendCommand(doc);
	                    })},
...
UStatus TestAgent::onReceive(uprotocol::utransport::UMessage &transportUMessage) const {
	...
	if (transportUMessage.attributes().type() == uprotocol::v1::UMessageType::UMESSAGE_TYPE_REQUEST) {
		...
		uprotocol::utransport::UPayload payload1((const unsigned char *) serialized_message.c_str(),
		    serialized_message.length(), uprotocol::utransport::UPayloadType::VALUE);
		...
		auto attr = uprotocol::utransport::UAttributesBuilder::response(transportUMessage.attributes().sink(),
		    transportUMessage.attributes().source(), UPriority::UPRIORITY_CS4, transportUMessage.attributes().id())
		                .build();
...
std::shared_ptr<uprotocol::utransport::UTransport> TestAgent::createTransport(const std::string &transportType) {
	if (transportType == "socket") {
		...
		return uprotocol::client::UpZenohClient::instance(BuildUAuthority().setName("cpp").build(),
