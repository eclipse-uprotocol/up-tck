...
public:
										...
										const uprotocol::utransport::UAttributes &attributes) const
	{
        ...
        if (attrV1.type() == uprotocol::v1::UMessageType::UMESSAGE_TYPE_REQUEST)
        {
        	...
        	std::cout << "SocketUListener::onReceive(), Received payload : " << payload.data() << std::endl;
        	std::cout << "SocketUListener::onReceive(), Received attributes : " << attributes.priority() << std::endl;
        	ustatus = m_transport->send(uri, payload, attributes);
        }
        else
        {
