...
struct TestUUri {
	...
	std::string to_string() const {
		return std::string("<< ") +
		       uprotocol::v1::UUri(*this).ShortDebugString() + " >>";
	}
...
void test_pub_sub(shared_ptr<SocketUTransport> transport) {
	...
	auto lhandle1 = transport->registerListener(
	    [&](const uprotocol::v1::UMessage& msg) {
...
void test_rpc_req(shared_ptr<SocketUTransport> transport) {
	...
	auto lhandle0 = transport->registerListener(
	    [&](const uprotocol::v1::UMessage& msg) {
		    cout << "#### got rpc req exact" << endl;
	    },
	    src, sink);

	// auto lhandle1 = transport->registerListener([&](const
	// uprotocol::v1::UMessage& msg) {
